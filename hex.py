...
XPAD = 40
YPAD = 40
WIN_HEIGHT = 2 * YPAD + GRID_SIZE * IMG_SIZE + 200
WIN_WIDTH = 2 * XPAD + (3 * GRID_SIZE - 1) * IMG_SIZE + 500
...
class playInfo():
    ...
    def __init__(self):
        self.board = [[-1 for x in range(GRID_SIZE + 5)] for x in \
                      range(GRID_SIZE + 5)]
        self.Par = [[data(0, 0) for x in range(GRID_SIZE + 5)] \
...
def resize_image(path,size):
    ...
    frame_width, frame_height = path.size
...
class gameGrid():
    def __init__(self, frame):
        ...
        empty_hex = resize_image(empty_hex0,50)
        sea_hex0 = Image.open('sea_hex.JPG')
        sea_hex = resize_image(sea_hex0,50)
        shipbody0 = Image.open('shipbody.JPG')
        shipbody = resize_image(shipbody0,50)
        self.frame = frame
        self.white = ImageTk.PhotoImage(empty_hex)
        self.red = ImageTk.PhotoImage(sea_hex)
        self.blue = ImageTk.PhotoImage(shipbody)
        self.drawGrid()
